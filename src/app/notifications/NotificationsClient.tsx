...
export default function NotificationsClient({ initialRows, mock = false }: Props) {
  ...
  return (
    <>
      ...
      {mock && (
        <div className="mb-6 px-3 py-2 text-xs text-neutral-500 dark:text-neutral-400 border border-dashed border-neutral-300 dark:border-neutral-700 rounded-lg">
        ...
        (["Today", "This week", "Earlier"] as const).map((bucket) => {
          ...
          return (
            <section key={bucket} className="mb-8">
              <h2 className="eyebrow mb-3">{bucket}</h2>
                ...
                {items.map((row) => {
                  ...
                  return (
                    <Link
                      key={row.id}
                      href={r.href}
                      className={`flex items-start gap-3 px-4 py-3 transition-colors hover:bg-ink-100 ${
                        unread ? "bg-ink-100/60" : ""
                      }`}
                    >
                      {r.actorAvatar ? (
                        <img
