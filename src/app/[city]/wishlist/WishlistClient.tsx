...
import { Bookmark, Search, X, ChevronLeft, ChevronRight, SlidersHorizontal } from "lucide-react";
import { Button } from "@/components/ui/Button";

const PER_PAGE_SM = 8;
const PER_PAGE_LG = 9;

export default function WishlistClient({ spots, citySlug }: { spots: Spot[]; citySlug: string }) {
          ...
          {filtered.length === 0 && spots.length === 0 ? (
            <div className="text-center py-16">
              <div className="w-12 h-12 rounded-full flex items-center justify-center mx-auto mb-4 bg-ink-100">
                <Bookmark size={20} strokeWidth={1.5} className="text-neutral-500 dark:text-neutral-400" />
              </div>
              <h3 className="text-base font-medium">Your wishlist is empty</h3>
              <p className="text-sm text-neutral-500 dark:text-neutral-400 mt-1.5 max-w-xs mx-auto">
                Tap the bookmark on any space to add it to your wishlist.
              </p>
