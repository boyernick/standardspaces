...
import { getSpotById, getSpotsByCity, spots } from "@/lib/data";
...
function InfoRow({ icon, label, value, href }: { icon: React.ReactNode; label: string; value: string; href?: string }) {
  const content = href ? (
    <a href={href} target="_blank" rel="noopener noreferrer" className="text-brand-900 dark:text-brand-400 hover:underline">
      {value}
    </a>
  ) : (
    <span>{value}</span>
...
export default async function SpotPage({
  ...
  return (
    <div className="h-[calc(100dvh-49px)] overflow-y-auto bg-white dark:bg-neutral-950">
      ...
      <div className="max-w-3xl mx-auto px-4 md:px-6">
        ...
        <nav className="flex items-center gap-2 text-sm text-neutral-400 dark:text-neutral-500 pt-6 pb-4">
          ...
          <span className="text-neutral-600 dark:text-neutral-300">{spot.name}</span>
        </nav>
        ...
        {spot.tags && spot.tags.length > 0 && (
          <div className="flex flex-wrap gap-2 mt-4">
            {spot.tags.map((tag) => (
              <span key={tag} className="text-xs px-2.5 py-1 rounded-full bg-neutral-100 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-400">
                {tag}
              </span>
            ))}
          </div>
          ...
          <>
            ...
            <div>
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 text-sm text-brand-900 dark:text-brand-400 hover:underline"
              >
